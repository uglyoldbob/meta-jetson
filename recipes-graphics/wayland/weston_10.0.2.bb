...
do_install:append() {
	...
	if [ "${@bb.utils.filter('DISTRO_FEATURES', 'x11', d)}" ]; then
		...
		install ${WORKDIR}/sources-unpack/weston.desktop ${D}${datadir}/applications

		install -d ${D}${datadir}/icons/hicolor/48x48/apps
		install ${WORKDIR}/sources-unpack/weston.png ${D}${datadir}/icons/hicolor/48x48/apps
	fi

	if [ "${@bb.utils.contains('PACKAGECONFIG', 'xwayland', 'yes', 'no', d)}" = "yes" ]; then
		install -Dm 644 ${WORKDIR}/sources-unpack/xwayland.weston-start ${D}${datadir}/weston-start/xwayland
	fi

	if [ "${@bb.utils.contains('PACKAGECONFIG', 'systemd', 'yes', 'no', d)}" = "yes" ]; then
		install -Dm 644 ${WORKDIR}/sources-unpack/systemd-notify.weston-start ${D}${datadir}/weston-start/systemd-notify
	fi
