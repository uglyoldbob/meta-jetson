...
do_install() {
	if [ ! -z "${SERIAL_CONSOLES}" ] ; then
		...
		install -m 0644 ${WORKDIR}/sources-unpack/serial-getty@.service ${D}${systemd_system_unitdir}/
		...
		for entry in $tmp ; do
			...
			if [ "$baudrate" = "$default_baudrate" ] ; then
				...
				install -m 0644 ${WORKDIR}/sources-unpack/serial-getty@.service ${D}${systemd_system_unitdir}/serial-getty$baudrate@.service
